...
def fixtures(request):
    ...
    response = utils.get_data(URL, "HandballAPI/data/fixtures.json", LIVE, STORE)
...
def get_contest_fixtures(request):
    URL = utils.create_request_url("/games", request.GET)
    response = utils.get_data(URL, "HandballAPI/data/contest_game.json", LIVE, STORE)
...
def get_contests(_):
    ...
    contests = utils.get_data(URL, "HandballAPI/data/handball_contests.json", LIVE, store=False)
    if LIVE:
        if STORE:
            utils.store_contests(contests)

        contests = utils.group_contests(contests)
...
def get_team_fixtures(request):
    URL = utils.create_request_url("/games", request.GET)
    response = utils.get_data(URL, "HandballAPI/data/team_fixtures.json", LIVE, STORE)
...
def get_standings(request):
    ...
    response = utils.get_data(URL, "HandballAPI/data/standings.json", LIVE, STORE)
...
def get_game(request):
    ...
    games = utils.get_data(URL, "HandballAPI/data/game.json", LIVE, STORE)
