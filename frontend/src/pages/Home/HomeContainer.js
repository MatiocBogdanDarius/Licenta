...
import {COUNTRIES, GAME_STATUS_FILTERS_VALUES, SPORTS} from "assets/constants/Data";
import { addDays } from 'date-fns';
import {getFormattedDate} from "utils/formatDate";
import * as SPORT_EVENT_AGGREGATOR from "services/api/sport_event_aggregator";
...
export function HomeContainer() {
    ...
    useEffect(() => {
        getContests();
    }, [gameStatusFilterValue, selectedDate])

    useEffect(() => {
        setCountries((isVisibleAllCountries ? COUNTRIES : COUNTRIES.slice(0, 40))
            .map(country => {
                return {
                    ...country, isVisibleContests: false
                }
            }));
    }, [isVisibleAllCountries])
